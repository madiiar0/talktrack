import { CheckCircle2, LoaderCircle, ShieldCheck } from 'lucide-react'
import { useState } from 'react'
import type { FormEvent } from 'react'
import PublicPageLayout, { PageLink } from './PublicPageLayout'
import { postJson } from '../lib/api'

type SubmitStatus = 'idle' | 'submitting' | 'success' | 'error'

const inputClass =
  'min-h-[52px] rounded-[16px] border border-[#B7E3F2] bg-[#fafdff] px-4 text-[15px] font-medium text-[#071014] outline-none transition-[border-color,box-shadow] placeholder:text-[#101820]/35 focus:border-[#2498C7] focus:shadow-[0_0_0_4px_rgba(36,152,199,0.16)]'

const labelClass = 'grid gap-2 text-[13px] font-bold text-[#071014]'

function isValidEmail(value: string) {
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)
}

export default function ExportDataPage() {
  const [name, setName] = useState('')
  const [accountEmail, setAccountEmail] = useState('')
  const [confirmAccountEmail, setConfirmAccountEmail] = useState('')
  const [details, setDetails] = useState('')
  const [confirmed, setConfirmed] = useState(false)
  const [company, setCompany] = useState('')
  const [status, setStatus] = useState<SubmitStatus>('idle')
  const [responseMessage, setResponseMessage] = useState('')

  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    if (status === 'submitting') return

    const email = accountEmail.trim().toLowerCase()
    const confirmation = confirmAccountEmail.trim().toLowerCase()

    if (!email || !isValidEmail(email)) {
      setStatus('error')
      setResponseMessage('Please enter a valid account email address.')
      return
    }

    if (email !== confirmation) {
      setStatus('error')
      setResponseMessage('The account email fields must match.')
      return
    }

    if (!confirmed) {
      setStatus('error')
      setResponseMessage(
        'Please confirm that TalkTrack may verify account ownership.',
      )
      return
    }

    setStatus('submitting')
    setResponseMessage('')

    try {
      const result = await postJson('/api/export-data', {
        name: name.trim(),
        accountEmail: email,
        confirmAccountEmail: confirmation,
        message: details.trim(),
        ownershipVerificationAcknowledged: confirmed,
        company,
      })

      setStatus('success')
      setResponseMessage(
        result.message || 'Your data export request has been received.',
      )
      setName('')
      setAccountEmail('')
      setConfirmAccountEmail('')
      setDetails('')
      setConfirmed(false)
      setCompany('')
    } catch (error) {
      setStatus('error')
      setResponseMessage(
        error instanceof Error
          ? error.message
          : 'Something went wrong. Please try again.',
      )
    }
  }

  return (
    <PublicPageLayout
      eyebrow="Data Export"
      title="Request your TalkTrack data."
      description="Submit a data export request for your TalkTrack account and logs. This creates a secure review task; it does not automatically send sensitive data."
    >
      <div className="mt-12 grid gap-8 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
        <aside className="rounded-[28px] border border-[#B7E3F2]/80 bg-white/78 p-6 shadow-[0_24px_88px_-76px_rgba(52,116,138,0.48)] backdrop-blur-sm">
          <div className="mb-5 grid h-12 w-12 place-items-center rounded-full bg-[#E7F6FB] text-[#147BA6]">
            <ShieldCheck size={24} strokeWidth={2.4} aria-hidden="true" />
          </div>
          <p className="text-[12px] font-bold uppercase tracking-[0.16em] text-[#147BA6]">
            What happens next
          </p>
          <div className="mt-5 grid gap-4 text-[15px] font-medium leading-[1.7] text-[#101820]/68">
            <p>
              We'll review your request and may need to verify account ownership
              before preparing or sending an export.
            </p>
            <p>
              This page does not automatically export data or send sensitive
              account information by email.
            </p>
            <p>
              For broader privacy details, read the{' '}
              <PageLink href="/policies">TalkTrack policies</PageLink>. For
              support questions, use <PageLink href="/contact">Contact</PageLink>.
            </p>
          </div>
        </aside>

        <form
          onSubmit={handleSubmit}
          className="rounded-[28px] border border-[#B7E3F2]/90 bg-white/82 p-5 shadow-[0_24px_88px_-76px_rgba(52,116,138,0.48)] backdrop-blur-sm sm:p-7"
        >
          <div className="grid gap-5">
            <label className={labelClass}>
              Name
              <input
                type="text"
                value={name}
                onChange={(event) => setName(event.target.value)}
                placeholder="Your name"
                maxLength={120}
                className={inputClass}
              />
            </label>

            <label className={labelClass}>
              Account email
              <input
                required
                type="email"
                value={accountEmail}
                onChange={(event) => setAccountEmail(event.target.value)}
                placeholder="email used for TalkTrack"
                maxLength={160}
                autoComplete="email"
                className={inputClass}
              />
            </label>

            <label className={labelClass}>
              Confirm account email
              <input
                required
                type="email"
                value={confirmAccountEmail}
                onChange={(event) => setConfirmAccountEmail(event.target.value)}
                placeholder="re-enter account email"
                maxLength={160}
                autoComplete="email"
                className={inputClass}
              />
            </label>

            <label className={labelClass}>
              Optional details
              <textarea
                value={details}
                onChange={(event) => setDetails(event.target.value)}
                placeholder="Anything else we should know about the request?"
                maxLength={2000}
                rows={5}
                className={`${inputClass} min-h-[132px] resize-y py-3 leading-[1.55]`}
              />
            </label>

            <label className="flex items-start gap-3 rounded-[18px] border border-[#B7E3F2]/85 bg-[#E7F6FB]/45 p-4 text-[14px] font-semibold leading-[1.55] text-[#101820]/72">
              <input
                required
                type="checkbox"
                checked={confirmed}
                onChange={(event) => setConfirmed(event.target.checked)}
                className="mt-1 h-4 w-4 accent-[#147BA6]"
              />
              <span>
                I understand TalkTrack may need to verify account ownership
                before processing or sending my data export.
              </span>
            </label>

            <label className="hidden" aria-hidden="true">
              Company
              <input
                type="text"
                tabIndex={-1}
                value={company}
                onChange={(event) => setCompany(event.target.value)}
                autoComplete="off"
              />
            </label>

            <button
              type="submit"
              disabled={status === 'submitting'}
              className="inline-flex min-h-[56px] items-center justify-center gap-2 rounded-full border border-[#050708]/80 bg-gradient-to-r from-[#2498C7] to-[#147BA6] px-6 text-[14px] font-bold uppercase tracking-[0.05em] text-white shadow-[0_18px_44px_-30px_rgba(38,114,143,0.55)] transition-[border-color,transform,box-shadow,opacity] duration-200 hover:-translate-y-0.5 hover:border-[#147BA6] hover:shadow-[0_18px_42px_-30px_rgba(36,152,199,0.72)] disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:translate-y-0"
            >
              {status === 'submitting' ? (
                <LoaderCircle size={17} strokeWidth={2.6} className="animate-spin" />
              ) : status === 'success' ? (
                <CheckCircle2 size={17} strokeWidth={2.6} />
              ) : (
                <ShieldCheck size={17} strokeWidth={2.6} />
              )}
              {status === 'submitting' ? 'Sending...' : 'Request export'}
            </button>

            {responseMessage ? (
              <p
                aria-live="polite"
                className={[
                  'rounded-[16px] border p-4 text-[14px] font-bold',
                  status === 'success'
                    ? 'border-[#B7E3F2] bg-[#E7F6FB]/70 text-[#147BA6]'
                    : 'border-red-200 bg-red-50 text-red-600',
                ].join(' ')}
              >
                {responseMessage}
              </p>
            ) : null}
          </div>
        </form>
      </div>
    </PublicPageLayout>
  )
}
